...
from training_loops import loop_vanilla
...
def prepare_textual_inversion(placeholder:str, tokenizer:object,text_encoder:object,initializer_token:str="person"):
...
def prepare_from_token_strategy(timesteps: torch.Tensor):
    return
...
def train_and_evaluate_one_sample_vanilla(
    ...
    if token_strategy==MULTI:
        for t in timesteps:
            ...
            token_dict[t.long().detach().tolist()]="<{}>".format(new_token)
    ...
    if token_strategy==DEFAULT:
        ...
        evaluation_image_list=[
            call_vanilla_with_dict(pipeline,evaluation_prompt.format(PLACEHOLDER),num_inference_steps=num_inference_steps,
                    negative_prompt=NEGATIVE,
    ...
    src_image_embed=outputs.image_embeds.detach().numpy()[-src_image_n:]
